...
class Det_Mir_Spider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        region_ids = []
        region_data = chompjs.parse_js_object(clean_string(response.xpath("//script[@type='text/template'][@id='app-data']/text()").get()))
    ...
    def parse_data(self, response, **kwargs):
        try:
            ...
            if len(data) != 0:
                ...
                logging.info(f'Ended parsing through category for region: {kwargs["region"]["city"]}')
